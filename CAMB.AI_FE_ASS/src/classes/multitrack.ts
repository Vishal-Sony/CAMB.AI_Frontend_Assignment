...
class MultiTrack extends EventEmitter<MultitrackEvents> {
  ...
  static create(
    ...
    return new MultiTrack(tracks, options,cacheAudios)
  }

  
  constructor(tracks: MultitrackTracks, options: MultitrackOptions,cacheAudios ) {
    super()
    ...
    this.initAllAudios(cacheAudios).then((durations) => {
  ...
  private async initAllAudios(cacheAudios): Promise<number[]> {
    if(cacheAudios.length){
      const remTracks= this.tracks.slice(cacheAudios.length - 1)
      console.log(remTracks);
      const remAudio = await Promise.all(remTracks.map((track) => this.initAudio(track)))  
      console.log(remAudio);
      
      this.audios = [
        ...cacheAudios.slice(0,-1),
        ...remAudio
      ];
    }
    else{
